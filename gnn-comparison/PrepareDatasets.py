...
#     else:
#         preprocess_dataset(dataset_name, args_dict)# Minimal dataset preparer for ENZYMES on modern PyG
import argparse
from pathlib import Path
from torch_geometric.datasets import TUDataset
from torch_geometric.transforms import NormalizeFeatures

def prepare_enzymes(root: str):
    root = Path(root)
    root.mkdir(parents=True, exist_ok=True)
    # download/process ENZYMES here
    _ = TUDataset(root=str(root), name="ENZYMES", use_node_attr=True,
                  transform=NormalizeFeatures())
    print(f"ENZYMES prepared under: {root}")

def main():
    ap = argparse.ArgumentParser()
    ap.add_argument('DATA_DIR', help='where to save the datasets')
    ap.add_argument('--dataset-name', default='ENZYMES', choices=['ENZYMES'])
    args = ap.parse_args()
    prepare_enzymes(args.DATA_DIR)

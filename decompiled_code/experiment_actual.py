...
class Experiment():
    def __init__(self, args):
        ...
        # Force num_workers=0 to avoid multiprocessing issues with PyG Data objects
        self.loader_workers = 0
        if args.loader_workers > 0:
            print(f'Warning: Forcing loader_workers=0 to avoid multiprocessing issues (was {args.loader_workers})')
    ...
    def run(self):
        ...
        optimizer.zero_grad()  # Initialize gradients before first backward
        ...
        for epoch in range(1, (self.max_epochs // self.eval_every) + 1):
            ...
            for sub_epoch in range(self.eval_every):
                pin_memory = self.device.type == 'cuda'
                loader = DataLoader(self.X_train, batch_size=self.batch_size, shuffle=True,
                                    pin_memory=pin_memory, num_workers=self.loader_workers,

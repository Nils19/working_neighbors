...
class GraphModel(torch.nn.Module):
    def __init__(self, gnn_type, num_layers, dim0, h_dim, out_dim, last_layer_fully_adjacent,
        ...
        if self.gnn_type is GNN_TYPE.GGNN:
            # Ignore unroll completely
            self.layers.append(GatedGraphConv(out_channels=h_dim, num_layers=num_layers))
        else:    
            if unroll:
        ...
        if self.use_layer_norm and gnn_type is not GNN_TYPE.GGNN:
            for i in range(num_layers):
